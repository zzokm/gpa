...
const GroupedCourseTable: React.FC<GroupedCourseTableProps> = ({ 
  ...
  const [isInitialized, setIsInitialized] = useState(false);  // Load group states from localStorage on component mount and initialize missing ones
  useEffect(() => {
    try {
  ...
  return (
    <div className="table-box">
      ...
      {Object.keys(nestedGroupedCourses).sort().map(level => {
        ...
        return (
          <div key={level} className="course-group level-group">            {/* Level Header */}
            <div 
            ...
            >
              <h3 className="group-title">
                ...
                <span className="group-name">{level}</span>                <button 
                  className="info-btn"
                  title={`GPA: ${levelStats.gpa.toFixed(2)}`}
                  aria-label={`GPA for ${level}: ${levelStats.gpa.toFixed(2)}`}
                  onMouseDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                  }}
            ...
            {/* Level Content */}
            <div className={`table-container level-container ${isLevelExpanded ? 'expanded' : 'collapsed'}`}>              {/* Terms within the level */}
              {Object.keys(nestedGroupedCourses[level])                .sort((a, b) => {
                  ...
                  // First sort by the defined term order - placing known terms first
                  if (orderA !== orderB) {
                ...
                return (
                  <div key={`${level}-${term}`} className="course-group term-group">                    {/* Term Header */}
                    <div 
                    ...
                    >
                      <h4 className="group-title term-title">
                        ...
                        <span className="group-name">{term}</span>                        <button 
                          className="info-btn"
                          title={`GPA: ${termStats.gpa.toFixed(2)}`}
                          aria-label={`GPA for ${term}: ${termStats.gpa.toFixed(2)}`}
                          onMouseDown={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                          }}
