...
import { parseCoursesFromHtml, mergeImportedCourses } from '../utils/courseParser';
...
const ImportModal: React.FC<ImportModalProps> = ({ show, entryPoint = 'other', onHide, onImport, currentCourses, onOpenHowTo }) => {
  ...
  const handleImport = () => {
    ...
    const importedCourses = parseCoursesFromHtml(pastedHTML);

    if (importedCourses.length > 0) {
      const finalCourses = mergeImportedCourses(importedCourses, currentCourses);
