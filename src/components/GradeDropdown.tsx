...
const GradeDropdown: React.FC<GradeDropdownProps> = ({ 
  ...
  const handleToggle = (e: React.MouseEvent) => {
    ...
    const event = new CustomEvent('closeAllDropdowns', { detail: { except: courseId } });
    document.dispatchEvent(event);
    
    setIsOpen(!isOpen);
  };
  // Listen for the closeAllDropdowns event
  useEffect(() => {
    const handleCloseAllDropdowns = (e: Event) => {
      // Type assertion for our custom event
      const customEvent = e as CustomEvent<{except: string}>;
      if (customEvent.detail?.except !== courseId) {
        setIsOpen(false);
      }
    };
